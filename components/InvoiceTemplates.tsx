...
function T01({ invoice, settings, accentColor, showLogo, fontFamily, fontSize }: Omit<Props, 'template'>) {
  ...
  return (
    <div style={base}>
      <div style={{ textAlign: 'center', marginBottom: '4px' }}>
        {showLogo && settings.logo_data_url && <img src={settings.logo_data_url} alt="logo" style={{ height: '20px', marginBottom: '4px' }} />}
        <div style={{ fontWeight: 700, fontSize: '1.1em' }}>{settings.business_name}</div>
      </div>
...
function T02({ invoice, settings, accentColor, showLogo, fontFamily, fontSize }: Omit<Props, 'template'>) {
  ...
  const label: React.CSSProperties = { fontSize: '0.7em', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#888' };
  return (
    <div style={base}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
        <div>
          {showLogo && settings.logo_data_url && <img src={settings.logo_data_url} alt="logo" style={{ height: '18px', display: 'block', marginBottom: '2px' }} />}
          <div style={{ fontWeight: 700, fontSize: '1em' }}>{settings.business_name}</div>
        </div>
      ...
      <div style={{ marginTop: '8px', borderTop: `2px solid ${accentColor}`, paddingTop: '6px', textAlign: 'right' }}>
        <span style={label}>Total Keseluruhan </span>
...
function T03({ invoice, settings, accentColor, showLogo, fontFamily, fontSize }: Omit<Props, 'template'>) {
    ...
    <div style={base}>
      <div style={{ background: accentColor, color: '#fff', padding: '8mm 8mm 6mm', marginBottom: '8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            {showLogo && settings.logo_data_url && <img src={settings.logo_data_url} alt="logo" style={{ height: '18px', display: 'block', marginBottom: '2px', filter: 'brightness(10)' }} />}
            <div style={{ fontWeight: 700, fontSize: '1.1em' }}>{settings.business_name}</div>
          </div>
          <div style={{ textAlign: 'right' }}>
...
function T04({ invoice, settings, accentColor, showLogo, fontFamily, fontSize }: Omit<Props, 'template'>) {
  ...
  const label: React.CSSProperties = { fontSize: '0.7em', textTransform: 'uppercase', color: '#888', marginBottom: '1px' };
  return (
    <div style={base}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
        ...
        <div style={col}>
          <div style={label}>No. Invoice</div>
          <div style={{ fontWeight: 600, fontSize: '0.85em' }}>{invoice.invoice_no || '—'}</div>
          <div style={{ ...label, marginTop: '4px' }}>Tanggal</div>
          <div>{invoice.invoice_date || '—'}</div>
        </div>
      </div>
      <ItemsTable items={invoice.items || []} accentColor={accentColor} />
...
function T05({ invoice, settings, accentColor, showLogo, fontFamily, fontSize }: Omit<Props, 'template'>) {
  ...
  return (
    <div style={base}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
        <div>
          {showLogo && settings.logo_data_url && <img src={settings.logo_data_url} alt="logo" style={{ height: '18px', display: 'block', marginBottom: '2px' }} />}
          <div style={{ fontWeight: 700, fontSize: '1.2em' }}>{settings.business_name}</div>
        </div>
